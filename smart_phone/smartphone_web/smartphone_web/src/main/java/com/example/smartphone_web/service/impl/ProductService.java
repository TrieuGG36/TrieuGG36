...
import com.example.smartphone_web.common.StatusProduct;
...
import com.example.smartphone_web.dto.respone.image.ImageRespone;
...
import com.example.smartphone_web.entity.*;
...
//    private final ImeiRepo imeiRepo;
    ...
    public String createProduct(ProductRequestAdd requestProduct) {
        if(requestProduct.getRomRequestAdds().size() == 0){
            ...
            return String.valueOf(new SmartPhoneExp(ConstansErrorCode.ROM_NOT_EXIST).getErrorMessage().getVn());
        }
    ...
    public String editProduct(ProductRequestEdit requestEdit) {
        ...
        if(categoryEntity == null){
            throw new SmartPhoneExp(ConstansErrorCode.CATEGORY_NOT_EXIST);
        }
...
//                            Long imei = imeiRepo.countByPropertyProductId(p.getId());
                            ...
                            productPropertyRespone.setColorName(p.getColorEntity().getValueColor());
                            productPropertyRespone.setStatus(p.getStatus());
                            List<ProductPropertyEntity> list1 = propertyProductRepo.findByDeleteFlagIsFalseAndPropertyProductId(p.getId());
                            productPropertyRespone.setImeiResponses(list1.stream().map(this::mapToImei).collect(Collectors.toList()));
                            productPropertyRespone.setCountImei(p.getId() == null ? 0L : p.getId());
                            if(list1.size() > 0){
                                check = 1;
                                productPropertyResponeList.add(productPropertyRespone);
                            }
                ...
                if(productPropertyEntityList != null && productPropertyEntityList.size() > 0 ){
                    for (ProductPropertyEntity p: productPropertyEntityList) {
                        if(p.getQuantity()  > 0){
                            ...
                            productPropertyRespone.setColorName(p.getColorEntity().getValueColor());
                            productPropertyRespone.setStatus(p.getStatus());
                            List<ProductPropertyEntity> list1 = propertyProductRepo.findByIdProductPropety(p.getId());
                            if(list1.size() > 0){
                                check = 1;
                                productPropertyResponeList.add(productPropertyRespone);
                            }
    ...
    public String deleteProduct(Long id) {
        ProductEntity entity = productRepo.findByIdAndDeleteFlagIsFalse(id);
        if(entity == null){
            return String.valueOf(new SmartPhoneExp(ConstansErrorCode.PRODUCT_NOT_EXIST).getErrorMessage().getVn());
        }
        entity.setDeleteFlag(true);
        productRepo.save(entity);
        return "ok";
   ...
   /* private ImeiResponse mapToImei(ProductPropertyEntity imeiEntity){
        return new ImeiResponse(String.valueOf(imeiEntity.getId()), imeiEntity.getId().toString());
    }*/
}
